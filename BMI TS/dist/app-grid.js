    ...
    for (let i = 1; i <= cols; i++) {
        for (let j = 1; j <= rows; j++) {
            ...
            if (prevWeight <= weight && nextWeight > weight && prevHeight <= height * 100 && nextHeight > height * 100) {
                const scale = 0.7;
                colorGrid(canvas, "#1d3557", i, j, marginLeft + ((1 - scale) / 2) * cellWidth, marginBottom - ((1 - scale) / 2) * cellHeight, cellWidth, cellHeight, bh, scale);
            }
        }
    }
    ctx.beginPath();
...
function colorGrid(canvas, color, i, j, marginLeft, marginBottom, cellWidth, cellHeight, bh, scale = 1) {
    const ctx = canvas.getContext("2d");
    ctx.fillStyle = `${color}`;
    ctx.fillRect(marginLeft + (i - 1) * cellWidth, bh - marginBottom - j * cellHeight, cellWidth * scale, cellHeight * scale);
}
//# sourceMappingURL=app-grid.js.map